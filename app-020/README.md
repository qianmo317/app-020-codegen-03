...
- **疏散图编辑与合规校验**：房间/设施绘制、沿走道路径的疏散距离（Dijkstra）、灭火器覆盖、出口数量、设施检查台账
- **疏散演练记录与统计**（顶部「疏散演练」）：
  - 每次演练记录：日期、警报（假设起火）时间、起火楼层/房间、参演人数
  - 按楼层：疏散指挥、开始/结束时间、实际用时、是否堵在楼道口
  - 路线关键节点（楼梯口/出口）逐个记通过时间，自动算「警报→节点1→节点2…」各段用时并标出全程最慢段
  - 多次演练并排对比：同楼层历次用时趋势（条形+快慢结论）、出口使用次数排行、老是排最后的楼层、各层堵口率
  - 问题可挂到具体房间/设施（或文字位置），下次演练时对老问题逐条复查（已改进/仍存在/加重），形成整改闭环
